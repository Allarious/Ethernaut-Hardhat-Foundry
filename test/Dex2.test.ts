...
describe("Dex Exploit", function () {
  ...
  before(async function () {
    ...
    await this.dex.add_liquidity(this.token1.address, INITIAL_DEX_BALANCE);
    await this.dex.add_liquidity(this.token2.address, INITIAL_DEX_BALANCE);
  ...
  it("Should drain the money from the Dex", async function exploit() {
    this.exploit = await (
      await ethers.getContractFactory("DexTwoHack", attacker)
    ).deploy(this.dex.address, this.token1.address, this.token2.address);
    await this.exploit.connect(attacker).exploit();
  });
  after(async function checkSolution() {
    expect(await this.token1.balanceOf(attacker.address)).to.gt(INITIAL_DEX_BALANCE);
    expect(await this.token2.balanceOf(attacker.address)).to.gt(INITIAL_DEX_BALANCE);

    expect(await this.token1.balanceOf(this.dex.address)).to.eq(0);
    expect(await this.token2.balanceOf(this.dex.address)).to.eq(0);
  });
});
